...
class LUT3D
{
 public:
  ...
  void set(size_t x, size_t y, size_t z, const T r_val, const T g_val, const T b_val) {
    if ((x < x_dim_) && 
        (y < y_dim_) && 
        (z < z_dim_)) {
      size_t idx = (x_dim_ * y_dim_) * z + x_dim_ * y + x;
...
///
bool LoadSPI3DFromFile(const std::string &filename, LUT3Df *lut, std::string *err = nullptr);
...
bool LoadSPI3DFromFile(const std::string &filename, LUT3Df *lut, std::string *err) {
  ...
  while (ifs.good() && read_count > 0)
  {
      ...
      if (sscanf(line.c_str(), "%d %d %d %f %f %f",
          &x_idx, &y_idx, &z_idx,
          &r_value, &g_value, &b_value) == 6)
      {
        lut->set(size_t(x_idx), size_t(y_idx), size_t(z_idx),
